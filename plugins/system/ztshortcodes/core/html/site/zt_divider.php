...
<div class="zt-divider-wrap">
    ...
    if ($type == "hr")
    {
        ?>style="background-color: <?php echo $color; ?>" <?php } ?>
         <?php
         if ($type == "thin" || $type == "fat" || $type == "dotted" || $type == "icon-type-1" || $type == "icon-type-2")
         {
             ?>style="border-color: <?php echo $color; ?>"<?php } ?>>
         <?php
         if ($attributes->get('type') == 'text-only' || $attributes->get('type') == 'icon-type-1' || $attributes->get('type') == 'icon-type-2')
         {
             ?>
            <a href="#" class="animate-top" <?php
            if ($type == "text-only")
            {
                ?>style="border-color: <?php echo $color; ?>"<?php } ?>>
                <?php
